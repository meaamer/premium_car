<script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBAidy6_mCUYIDe90UtEvoNWnwzmhU3HoQ&libraries=places"></script>
    ...
    <section class="content">
      ...
      <div class="box">
        ...
        <form id="os_form">
            ...
            <input type="number" class="form-control" placeholder="Outstation Per Kilometer Charges" 
            name="os_per_km" value="<?php if( isset($rows) ){echo $rows['outstation_per_kilometer'];}?>">
              </div>
              <br>
			</div>
            ...
            <input type="number" class="form-control" placeholder="Extra per Kilometer Charges" 
            name="extra_per_km" value="<?php if( isset($rows) ){echo $rows['extra_per_kilometer'];}?>">
              </div>
              <br>
             </div>
			 	<div class="col-md-6">
        ...
        <div class="box-footer">
                ...
                <?php if (isset($rows)): ?>
    				...
    				onclick="update_os(<?php echo $rows['vendor_outstation_id'];?>);">
    				 Update</button>
                 
                <?php else: ?>
                
                		<button type="button" class="btn btn-primary pull-right" onclick="add_os();">
                		Add</button>
                <?php endif ?>	
                </form>
                <div id="error"></div>
  ...
  function GetCabs(){
	...
	if(cab_id==null){
  	...
  	$.ajax({
      type:"POST",
      url:"<?php echo base_url()?>VendorPricing/GetOSPricing",
  	 ...
  	 function add_os()
  {
    ...
    $.ajax({
      type:"POST",
      url:"<?php echo base_url()?>VendorPricing/AddOS",
      data:form.serialize(),
      success: function(response){
  ...
  function update_os(id)
  {
    //alert(id);
    var form =$("#os_form");
    $.ajax({
      type:"POST",
      url:"<?php echo base_url()?>VendorPricing/UpdateOS/"+id,
      data:form.serialize(),
      success: function(response){
      	//$('#error').html(response);
       alertify.success(response);
